...
foreach my$id (keys %gene_list) {
	...
	while (my $intron = shift @introns) {
		...
		print file_out_intron $intron_id."\t".$canonical_id."(".$canonical_transcript->cdna_coding_start.":".$canonical_transcript->cdna_coding_end.")"."\t".$id."\t"."chr".$seq_region.$strand.$coords_for_one_intron."\n";
	}
	...
	while (my $exon = shift @exons) {
		...
		print file_out_exon $exon->stable_id()."\t".$canonical_id."(".$canonical_transcript->cdna_coding_start.":".$canonical_transcript->cdna_coding_end.")"."\t".$id."\t"."chr".$seq_region.$strand.$coords_for_one_exon."\n";
	}
